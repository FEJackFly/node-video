...
app.post('/getHis', function (req, res) {
    ...
    var hisRtsp = 'rtsp://admin:jidian123@172.16.151.230:554/Streaming/tracks/' + channel + '01?starttime=20190917t221010z&endtime=20190917t221030z?transportmode=multicast&playType=his'
    console.log(hisRtsp)
    if (hisPlay != null) {
        hisPlay.stop()
    }
...
function HkIpcInit(nvr, num) {
    for (let i = 1; i <= num; i++) {
        ...
        let data = {
            rtsp: "rtsp://" + nvr + "/h264/ch" + parseInt(i + 32) + "/" + sub + "/av_stream",
            name: '通道' + i,
            channel: i,
            type: '海康NVR',
...
function DhIpcInit(nvr, num) {
    for (let i = 1; i <= num; i++) {
        ...
        let data = {
            rtsp: "rtsp://" + nvr + "/cam/realmonitor?channel=" + i + "&subtype=" + subtype,
            name: '通道' + i,
            channel: i,
            type: '大华NVR',
            coverSrc: 'https://www.jlrbjb.com/uploads/20180818065b77fc008e5b5.jpg',
